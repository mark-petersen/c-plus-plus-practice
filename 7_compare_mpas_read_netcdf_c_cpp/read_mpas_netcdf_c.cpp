...
int main() {
  ...
  string varName = "temperature";
  {
    ...
    if ((retval = nc_inq_varid(ncid, "temperature", &varid)))
      ERR(retval);
    if ((retval = nc_get_var_double(ncid, varid, &temperature[0][0])))
      ERR(retval);
    // netcdf c++ version:
  ...
  }
  {
    ...
    if ((retval = nc_inq_varid(ncid, varName.c_str(), &varid)))
      ERR(retval);
    if ((retval = nc_get_var_double(ncid, varid, &tempArray[0][0])))
      ERR(retval);
    for (int i = 0; i < dim1; i++) {
  ...
  }
  {
    ...
    if ((retval = nc_inq_dimid(ncid, "nCells", &dimid)))
      ERR(retval);
    if ((retval = nc_inq_dimlen(ncid, dimid, &dim1)))
      ERR(retval);
    size_t dim2;
    if ((retval = nc_inq_dimid(ncid, "nVertLevels", &dimid)))
      ERR(retval);
    if ((retval = nc_inq_dimlen(ncid, dimid, &dim2)))
      ERR(retval);
