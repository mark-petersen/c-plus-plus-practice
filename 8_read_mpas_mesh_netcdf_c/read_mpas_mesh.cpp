...
public:
  ...
  MpasMesh(string meshFileName) {
    ...
    if (verbose)
      cout << "** Opening file: " << meshFileName << " **" << endl;
    ...
    if (verbose)
      cout << "** Read in mesh variables **" << endl;
    ...
    // triangleAngleQuality = readNCDouble(ncid, "triangleAngleQuality",
    // nCells); boundaryVertex = readNCInt(ncid, "boundaryVertex", nVertices);
    // obtuseTriangle = readNCInt(ncid, "obtuseTriangle", nCells);
    if (verbose)
      cout << endl;

    if (verbose)
      cout << "** Read in state variables **" << endl;
